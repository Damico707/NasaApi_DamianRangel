 ...
 */
const API_KEY = "D6qYVsw2bRNcnQzJethWN5jQgeTaDH083dS35mNQ";
...
function openModal(data) {
    ...
    const isLater = checkIfLater(data.date);
    ...
    document.getElementById("modalBody").innerHTML = `
        ...
        <div style="display:flex; gap:10px; flex-wrap:wrap;">
            ...
            <button class="btn" style="background:${isLater ? '#f97316':'#fbbf24'}; color:#000;" 
                onclick='toggleLater(${JSON.stringify(data).replace(/'/g, "&apos;")})'>
                ${isLater ? '✅ Ya está guardada' : '🔖 Guardar para investigar después'}
            </button>
...
/**
 * GESTIÓN DE "VER DESPUÉS / INVESTIGAR" (LOCALSTORAGE)
 */
function getLater() {
    return JSON.parse(localStorage.getItem("nasa_later")) || [];
}

function checkIfLater(date) {
    return getLater().some(f => f.date === date);
}

function toggleLater(data) {
    let later = getLater();
    const index = later.findIndex(f => f.date === data.date);

    if (index === -1) {
        later.push(data);
        showToast("🔖 Guardada para investigar después");
    } else {
        later.splice(index, 1);
        showToast("Eliminada de la lista");
    }

    localStorage.setItem("nasa_later", JSON.stringify(later));
    closeModal();
    if (gallery.dataset.view === "later") showLater();
}

function showLater() {
    gallery.dataset.view = "later";
